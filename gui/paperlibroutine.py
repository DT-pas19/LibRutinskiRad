# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'paperlibroutine.ui'
#
# Created by: PyQt5 UI code generator 5.12.2
#
# WARNING! All changes made in this file will be lost!

from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(710, 600)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.gridLayout_2 = QtWidgets.QGridLayout(self.centralwidget)
        self.gridLayout_2.setObjectName("gridLayout_2")
        self.verticalLayout = QtWidgets.QVBoxLayout()
        self.verticalLayout.setObjectName("verticalLayout")
        self.horizontalLayout_2 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_2.setObjectName("horizontalLayout_2")
        self.lbl_path_caption = QtWidgets.QLabel(self.centralwidget)
        self.lbl_path_caption.setObjectName("lbl_path_caption")
        self.horizontalLayout_2.addWidget(self.lbl_path_caption)
        self.edit_path = QtWidgets.QLineEdit(self.centralwidget)
        self.edit_path.setObjectName("edit_path")
        self.horizontalLayout_2.addWidget(self.edit_path)
        self.btn_filelist_refresh = QtWidgets.QPushButton(self.centralwidget)
        self.btn_filelist_refresh.setText("")
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(":/refresh/view-refresh.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_filelist_refresh.setIcon(icon)
        self.btn_filelist_refresh.setObjectName("btn_filelist_refresh")
        self.horizontalLayout_2.addWidget(self.btn_filelist_refresh)
        self.btn_file_dialog = QtWidgets.QPushButton(self.centralwidget)
        self.btn_file_dialog.setMaximumSize(QtCore.QSize(30, 16777215))
        self.btn_file_dialog.setText("")
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap(":/open/folder-open.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_file_dialog.setIcon(icon1)
        self.btn_file_dialog.setObjectName("btn_file_dialog")
        self.horizontalLayout_2.addWidget(self.btn_file_dialog)
        self.verticalLayout.addLayout(self.horizontalLayout_2)
        self.tab_widget = QtWidgets.QTabWidget(self.centralwidget)
        self.tab_widget.setObjectName("tab_widget")
        self.tab_files = QtWidgets.QWidget()
        self.tab_files.setObjectName("tab_files")
        self.gridLayout_3 = QtWidgets.QGridLayout(self.tab_files)
        self.gridLayout_3.setObjectName("gridLayout_3")
        self.table_files_view = QtWidgets.QTableView(self.tab_files)
        self.table_files_view.setObjectName("table_files_view")
        self.gridLayout_3.addWidget(self.table_files_view, 0, 1, 1, 2)
        self.btn_start_reading = QtWidgets.QToolButton(self.tab_files)
        self.btn_start_reading.setMaximumSize(QtCore.QSize(90, 16777215))
        self.btn_start_reading.setLayoutDirection(QtCore.Qt.RightToLeft)
        icon2 = QtGui.QIcon()
        icon2.addPixmap(QtGui.QPixmap(":/direction/go-next.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_start_reading.setIcon(icon2)
        self.btn_start_reading.setToolButtonStyle(QtCore.Qt.ToolButtonTextBesideIcon)
        self.btn_start_reading.setObjectName("btn_start_reading")
        self.gridLayout_3.addWidget(self.btn_start_reading, 1, 2, 1, 1)
        self.tab_widget.addTab(self.tab_files, "")
        self.tab_students = QtWidgets.QWidget()
        self.tab_students.setEnabled(False)
        self.tab_students.setObjectName("tab_students")
        self.gridLayout_4 = QtWidgets.QGridLayout(self.tab_students)
        self.gridLayout_4.setObjectName("gridLayout_4")
        self.lbl_common_info = QtWidgets.QLabel(self.tab_students)
        self.lbl_common_info.setText("")
        self.lbl_common_info.setObjectName("lbl_common_info")
        self.gridLayout_4.addWidget(self.lbl_common_info, 0, 0, 1, 1)
        self.table_students_view = QtWidgets.QTableView(self.tab_students)
        self.table_students_view.setObjectName("table_students_view")
        self.gridLayout_4.addWidget(self.table_students_view, 1, 0, 1, 5)
        self.btn_back_choosing = QtWidgets.QToolButton(self.tab_students)
        self.btn_back_choosing.setMaximumSize(QtCore.QSize(70, 16777215))
        icon3 = QtGui.QIcon()
        icon3.addPixmap(QtGui.QPixmap(":/direction/go-previous.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_back_choosing.setIcon(icon3)
        self.btn_back_choosing.setToolButtonStyle(QtCore.Qt.ToolButtonTextBesideIcon)
        self.btn_back_choosing.setObjectName("btn_back_choosing")
        self.gridLayout_4.addWidget(self.btn_back_choosing, 2, 1, 2, 1)
        self.btn_save = QtWidgets.QToolButton(self.tab_students)
        self.btn_save.setMaximumSize(QtCore.QSize(100, 16777215))
        icon4 = QtGui.QIcon()
        icon4.addPixmap(QtGui.QPixmap(":/actions/document-save.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_save.setIcon(icon4)
        self.btn_save.setToolButtonStyle(QtCore.Qt.ToolButtonTextBesideIcon)
        self.btn_save.setObjectName("btn_save")
        self.gridLayout_4.addWidget(self.btn_save, 2, 3, 2, 1)
        self.btn_clipboard_save = QtWidgets.QToolButton(self.tab_students)
        self.btn_clipboard_save.setMaximumSize(QtCore.QSize(150, 16777215))
        icon5 = QtGui.QIcon()
        icon5.addPixmap(QtGui.QPixmap(":/clipboard/edit-copy.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_clipboard_save.setIcon(icon5)
        self.btn_clipboard_save.setToolButtonStyle(QtCore.Qt.ToolButtonTextBesideIcon)
        self.btn_clipboard_save.setObjectName("btn_clipboard_save")
        self.gridLayout_4.addWidget(self.btn_clipboard_save, 2, 4, 1, 1)
        self.btn_clipboard_open = QtWidgets.QToolButton(self.tab_students)
        icon6 = QtGui.QIcon()
        icon6.addPixmap(QtGui.QPixmap(":/clipboard/edit-paste.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_clipboard_open.setIcon(icon6)
        self.btn_clipboard_open.setToolButtonStyle(QtCore.Qt.ToolButtonTextBesideIcon)
        self.btn_clipboard_open.setObjectName("btn_clipboard_open")
        self.gridLayout_4.addWidget(self.btn_clipboard_open, 3, 4, 1, 1)
        self.btn_change = QtWidgets.QToolButton(self.tab_students)
        self.btn_change.setMaximumSize(QtCore.QSize(135, 16777215))
        icon7 = QtGui.QIcon()
        icon7.addPixmap(QtGui.QPixmap(":/actions/document-edit.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_change.setIcon(icon7)
        self.btn_change.setToolButtonStyle(QtCore.Qt.ToolButtonTextBesideIcon)
        self.btn_change.setObjectName("btn_change")
        self.gridLayout_4.addWidget(self.btn_change, 2, 2, 2, 1)
        self.tab_widget.addTab(self.tab_students, "")
        self.verticalLayout.addWidget(self.tab_widget)
        self.gridLayout_2.addLayout(self.verticalLayout, 0, 0, 1, 1)
        MainWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(MainWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 710, 21))
        self.menubar.setObjectName("menubar")
        MainWindow.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)

        self.retranslateUi(MainWindow)
        self.tab_widget.setCurrentIndex(0)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "Библиотечная рутина 451F"))
        self.lbl_path_caption.setText(_translate("MainWindow", "Исходная директория"))
        self.btn_start_reading.setText(_translate("MainWindow", "Далее"))
        self.tab_widget.setTabText(self.tab_widget.indexOf(self.tab_files), _translate("MainWindow", "Файлы"))
        self.btn_back_choosing.setText(_translate("MainWindow", "Назад"))
        self.btn_save.setText(_translate("MainWindow", "Сохранить"))
        self.btn_clipboard_save.setText(_translate("MainWindow", "В буфер обмена"))
        self.btn_clipboard_open.setText(_translate("MainWindow", "Из буфера обмена"))
        self.btn_change.setText(_translate("MainWindow", "Изменить общее"))
        self.tab_widget.setTabText(self.tab_widget.indexOf(self.tab_students), _translate("MainWindow", "Работы"))


import gui.resources_rc

if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    MainWindow = QtWidgets.QMainWindow()
    ui = Ui_MainWindow()
    ui.setupUi(MainWindow)
    MainWindow.show()
    sys.exit(app.exec_())
